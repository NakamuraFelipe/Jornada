...
class _GestaoUsuariosPageState extends State<GestaoUsuariosPage> {
  ...
  void _abrirCriarUsuario() {
    ...
    String? validarSenhaForte(String? v) {
      final senha = v?.trim() ?? '';

      if (senha.isEmpty) return 'Informe a senha';
      if (senha.length < 12) return 'A senha deve ter no mínimo 12 caracteres';
      if (senha.length > 15) return 'A senha deve ter no máximo 15 caracteres';
      ...
      if (!temNumero) return 'A senha deve conter pelo menos 1 número';
      if (!temEspecial)
    ...
    String? validarEmail(String? v) {
      ...
      if (!emailReg.hasMatch(email)) return 'Informe um e-mail válido';
      return null;
    }

    String? validarTelefone(String? v) {
      final tel = v?.trim() ?? '';
      if (tel.isEmpty) return 'Informe o telefone';
      if (tel.length < 8) return 'Telefone inválido';
      return null;
    }
                    ...
                    : () async {
                        ...
                        try {
                          final resp = await http.put(
                            Uri.parse(
                              '$kBaseUrl/gestor/usuario/${usuario.idUsuario}/resetar_senha',
                            ),
                            headers: {
                              'Content-Type': 'application/json',
                              if (_token != null)
                          ...
                          if (resp.statusCode == 200) {
                            ...
                            final data = jsonDecode(resp.body);
                            _showError(
                              data['mensagem'] ?? 'Erro ao resetar senha',
                            );
                          }
                        } catch (e) {
                          _showError('Erro de conexão: $e');
  ...
  void _confirmarDeletar(UsuarioConsultor usuario) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Deseja deletar o usuário "${usuario.nomeUsuario}"?\n\nEssa ação não pode ser desfeita.',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
